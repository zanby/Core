...
*/
class Warecorp_Facebook_User extends Warecorp_Data_Entity {
     ...
     */
    static public function registerUser(Warecorp_User $user) {
        $accounts = array();
        $accounts[] = array(
            'email_hash' => Warecorp_Facebook_User::hashEmail($user->getEmail()),
            'account_id' => $user->getId(),
            'account_url' => $user->getUserPath('profile')
        );
        $session_key = Warecorp_Facebook_Api::getInstance()->api_client->session_key;
        Warecorp_Facebook_Api::getInstance()->api_client->session_key = null;
        
        $connect_registerUsers = Warecorp_Facebook_Api::getInstance()->api_client->connect_registerUsers(json_encode($accounts));
        Warecorp_Facebook_Api::getInstance()->api_client->session_key = $session_key;
        return true;
    }
     ...
     */
    static public function unregisterUser(Warecorp_User $user) {
        $email_hash[] = Warecorp_Facebook_User::hashEmail($user->getEmail());
        $connect_registerUsers = Warecorp_Facebook_Api::getInstance()->api_client->connect_unregisterUsers(json_encode($email_hash));
        return true;
    }
    ...
    static function createUniqLogin($facebookInfo) {        
        if ( $facebookInfo['username'] && !Warecorp_User::isUserExists('login', $facebookInfo['username']) ) return $facebookInfo['username'];
