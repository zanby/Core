    ...
    static public function postStream($message, $attachment = null, $action_links = null, $target_id = null, $uid = null) {
        if ( FACEBOOK_USED ) {
            ...
            if ( !empty($facebookId) && Warecorp_Facebook_User::isFBAccountConnected($facebookId) ) {
                ...
                $isFBSend = false;
                if ( self::$allowDirectStream && Warecorp_Facebook_User::canPublishStream() ) {
                    try {
                        Warecorp_Facebook_Api::getInstance()->api_client->stream_publish($message['message'], $attachment, $action_links, $target_id, $uid);
                        $isFBSend = true;   
                    } catch (Exception $ex) {}                         
                }
                if ( !$isFBSend ) {                
    ...
    static public function postFeed($message, $template_data, $target_ids = '', $body_general = '', $story_size = FacebookRestClient::STORY_SIZE_SHORT, $user_message = '') {
        if ( FACEBOOK_USED ) {
            ...
            if ( !empty($facebookId) && Warecorp_Facebook_User::isFBAccountConnected($facebookId) ) {
                ...
                $isFBSend = false;
                if ( self::$allowDirectFeed && Warecorp_Facebook_User::canPublishStream() ) {
                    try {
        ...
        if ( FACEBOOK_USED ) {
            ...
            if ( !empty($facebookId) && Warecorp_Facebook_User::isFBAccountConnected($facebookId) ) {
                ...
                //if ( self::$allowDirectFeed && Warecorp_Facebook_User::canEmail() ) {
                    try {                     
                        Warecorp_Facebook_Api::getInstance()->api_client->notifications_sendEmail($recipients, $subject, $text, $fbml);
                        $isFBSend = true;  
                    } catch (Exception $ex) {}                         
                //}
                return $isFBSend;
